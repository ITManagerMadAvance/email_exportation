#!/usr/bin/env python3
"""
extract_banque.py -- "Extraction Banque"

Flow separe (dossier SharePoint et script dedies).

Le mot "banque" seul est trop generique (beaucoup de faux positifs sans
rapport, observe en pratique). Le vrai pattern trouve dans les pieces
jointes reelles est plutot "RELEVE BANCAIRE" / "RELEVE BQ" (releves
bancaires), ex: "RELEVE BQ 01 AOUT 2025 AU 31 AOUT 2025.pdf".

Le filtre declenche sur (objet OU nom de fichier, insensible aux
accents/casse) :
    - expression "releve bancaire" ou "releve bq"
    - mot "bancaire" en token entier

Cible par defaut : admin@madavance.org, la ou les exemples reels ont ete
trouves (releves transferes par olivia@madavance.org notamment).
...
Destination SharePoint :
    SHAREPOINT_FOLDER_LINK_BANQUE

Usage :
    python extract_banque.py
"""
...
def run_extraction(session: GraphSession, senders: list[str], output_dir: str, sharepoint_link: str | None) -> dict:
    ...
    def keyword_matches(text: str) -> bool:
        return token_matches(text) or phrase_matches(text)

    print(f"Filtre actif : objet OU nom de fichier contenant {TOKEN_KEYWORDS} ou {PHRASE_KEYWORDS} (accents ignores).")
    ...
    for mailbox in senders:
        ...
        for msg in messages:
            ...
            kept = [a for a in file_attachments if subject_matches or keyword_matches(a.get("name") or "")]
            if not kept:
                continue

            print(f"  [{received}] {subject} - {len(kept)}/{len(file_attachments)} piece(s) jointe(s) retenue(s)")
...
def build_summary_text(stats: dict) -> str:
    lines = [f"Extraction Banque terminee le {date.today().isoformat()}."]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Extraction Banque : pieces jointes filtrees de plusieurs boites mail donnees.")
    parser.add_argument("--senders", nargs="+", default=DEFAULT_SENDERS)
    parser.add_argument("--output-dir", default="./banque")
    parser.add_argument("--sharepoint-link", default=os.environ.get("SHAREPOINT_FOLDER_LINK_BANQUE"))
        ...
        if email_enabled:
            try:
                send_email(session, args.email_sender, args.email_recipients,
                           f"[ECHEC] Extraction Banque - {date.today().isoformat()}",
                           f"L'extraction Banque a echoue.\n\nErreur :\n{error_text}")
    ...
    if email_enabled:
        try:
            send_email(session, args.email_sender, args.email_recipients,
                       f"Extraction Banque - {date.today().isoformat()} : {stats['total_files']} fichier(s)",
