#!/usr/bin/env python3
"""
extract_mvola.py -- "Extraction MVOLA"

Flow separe (dossier SharePoint et script dedies).

Le mot "MVOLA" apparait rarement tel quel dans les objets/noms de fichiers
reels observes -- les vrais signaux trouves en pratique (recherche dans
admin@madavance.org) sont :
    - expediteur exact "virement@mvola.mg" (notifications automatiques MVOLA)
    - objet du type "CASH-IN 22 195 115 Ar"
    - pieces jointes nommees "CASH IN ... .png" / "AVIS DE DEBIT ... .png"
      (captures d'ecran de l'appli MVOLA, transferees par les collaborateurs)

Le filtre declenche donc sur PLUSIEURS signaux combines (n'importe lequel
suffit) :
    - expediteur se terminant par "@mvola.mg"
    - mot "mvola" en token entier (au cas ou il apparaisse explicitement)
    - expression "cash-in" / "cash in" / "avis de debit" (comparaison
      insensible aux accents/casse, sur objet ou nom de fichier)

Cible par defaut : admin@madavance.org, la ou les exemples reels ont ete
trouves. Modifiable via --senders.
...
Destination SharePoint :
    SHAREPOINT_FOLDER_LINK_MVOLA

Usage :
    python extract_mvola.py
"""
...
TOKEN_KEYWORDS = ["mvola"]
PHRASE_KEYWORDS = ["cash-in", "cash in", "avis de debit"]
SENDER_DOMAIN_SUFFIX = "@mvola.mg"
...
def make_token_matcher(keywords: list[str]):
    norm_keywords = [normalize_text(k) for k in keywords if k]
    patterns = [re.compile(rf"^{re.escape(k)}\d*$") for k in norm_keywords]

    def matcher(text: str) -> bool:
        if not text:
            return False
        tokens = re.split(r"[^A-Za-z0-9]+", normalize_text(text))
        return any(p.match(t) for t in tokens if t for p in patterns)

    return matcher


def make_phrase_matcher(phrases: list[str]):
    """Sous-chaine insensible aux accents/casse (pour des expressions comme
    'cash-in' ou 'avis de debit', qui ne sont pas des tokens uniques)."""
    norm_phrases = [normalize_text(p) for p in phrases if p]

    def matcher(text: str) -> bool:
        if not text:
            return False
        norm = normalize_text(text)
        return any(p in norm for p in norm_phrases)
...
def is_mvola_match(subject: str, attachment_name: str, sender_address: str,
                    token_matches, phrase_matches) -> bool:
    if sender_address and sender_address.lower().endswith(SENDER_DOMAIN_SUFFIX):
        return True
    for text in (subject, attachment_name):
        if token_matches(text) or phrase_matches(text):
            return True
    return False


def run_extraction(session: GraphSession, senders: list[str], output_dir: str, sharepoint_link: str | None) -> dict:
    token_matches = make_token_matcher(TOKEN_KEYWORDS)
    phrase_matches = make_phrase_matcher(PHRASE_KEYWORDS)
    print(f"Filtre actif : expediteur '*{SENDER_DOMAIN_SUFFIX}', OU mot-cle {TOKEN_KEYWORDS}, "
          f"OU expression {PHRASE_KEYWORDS} (objet ou nom de fichier).")
    ...
    for mailbox in senders:
        ...
        for msg in messages:
            ...
            subject_matches = is_mvola_match(subject, "", sender_address, token_matches, phrase_matches)
            kept = [
                a for a in file_attachments
                if subject_matches or is_mvola_match("", a.get("name") or "", sender_address, token_matches, phrase_matches)
            ]
            if not kept:
                continue
...
def build_summary_text(stats: dict) -> str:
    lines = [f"Extraction MVOLA terminee le {date.today().isoformat()}."]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Extraction MVOLA : pieces jointes filtrees de plusieurs boites mail donnees.")
    parser.add_argument("--senders", nargs="+", default=DEFAULT_SENDERS)
    parser.add_argument("--output-dir", default="./mvola")
    parser.add_argument("--sharepoint-link", default=os.environ.get("SHAREPOINT_FOLDER_LINK_MVOLA"))
        ...
        if email_enabled:
            try:
                send_email(session, args.email_sender, args.email_recipients,
                           f"[ECHEC] Extraction MVOLA - {date.today().isoformat()}",
                           f"L'extraction MVOLA a echoue.\n\nErreur :\n{error_text}")
    ...
    if email_enabled:
        try:
            send_email(session, args.email_sender, args.email_recipients,
                       f"Extraction MVOLA - {date.today().isoformat()} : {stats['total_files']} fichier(s)",
