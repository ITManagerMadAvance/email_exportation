#!/usr/bin/env python3
"""
extract_decaissement.py -- "Extraction Decaissement"

Flow separe (dossier SharePoint et script dedies, independant des autres
extractions OV / Factures / Proforma / MVOLA / Banque).

Critere observe en pratique (recherche dans admin@madavance.org) : les
demandes de decaissement ont des objets explicites du type "Demande de
decaissement - Equipe MERV Sensors". Le filtre reconnait le mot "decaissement"
et ses variantes proches ("decaisse", "decaisser"), independamment des
accents (comparaison normalisee : "decaissement" == "décaissement").

Cible par defaut : admin@madavance.org, la ou l'exemple reel a ete trouve.
...
Destination SharePoint :
    SHAREPOINT_FOLDER_LINK_DECAISSEMENT

Usage :
    python extract_decaissement.py
    python extract_decaissement.py --senders admin@madavance.org antonio@madavance.org
"""
...
KEYWORDS = ["decaissement", "decaisse", "decaisser"]
...
def normalize_text(value: str) -> str:
    """Minuscule + sans accents, pour comparer 'decaissement' et 'décaissement'."""
...
def make_keyword_matcher(keywords: list[str]):
    """Token entier (avec suffixe chiffre optionnel), insensible aux accents/casse."""
...
def run_extraction(session: GraphSession, senders: list[str], output_dir: str, sharepoint_link: str | None) -> dict:
    ...
    print(f"Filtre actif : objet OU nom de fichier contenant {KEYWORDS} (accents ignores).")
...
def build_summary_text(stats: dict) -> str:
    lines = [f"Extraction Decaissement terminee le {date.today().isoformat()}."]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Extraction Decaissement : pieces jointes filtrees de plusieurs boites mail donnees.")
    parser.add_argument("--senders", nargs="+", default=DEFAULT_SENDERS)
    parser.add_argument("--output-dir", default="./decaissement")
    parser.add_argument("--sharepoint-link", default=os.environ.get("SHAREPOINT_FOLDER_LINK_DECAISSEMENT"))
        ...
        if email_enabled:
            try:
                send_email(session, args.email_sender, args.email_recipients,
                           f"[ECHEC] Extraction Decaissement - {date.today().isoformat()}",
                           f"L'extraction Decaissement a echoue.\n\nErreur :\n{error_text}")
    ...
    if email_enabled:
        try:
            send_email(session, args.email_sender, args.email_recipients,
                       f"Extraction Decaissement - {date.today().isoformat()} : {stats['total_files']} fichier(s)",
